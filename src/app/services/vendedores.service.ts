...
export class VendedoresService {
  ...
  constructor(private firestore: AngularFirestore, private toolboxService: ToolboxService, private router: Router) {
    this.itemsCollection = this.firestore.collection('vendedores');
  }
  ...
  async updateItem(id: any, newData: any): Promise<void> {
    ...
    try {
      const users = await firstValueFrom(this.firestore.collection('vendedores', ref => ref.where('cpf', '==', cpf)).valueChanges({ idField: 'id' }));
